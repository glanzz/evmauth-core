...
import {IERC1155PurchaseWithERC20} from "./extensions/IERC1155PurchaseWithERC20.sol";
import {IERC20} from "@openzeppelin/contracts/token/ERC20/IERC20.sol";
import {SafeERC20} from "@openzeppelin/contracts/token/ERC20/utils/SafeERC20.sol";
import {IERC165} from "@openzeppelin/contracts/utils/introspection/IERC165.sol";

/**
 * @dev Extension of ERC1155AccessControlPrice that adds ERC20 token purchase functionality.
 * This avoids diamond inheritance by extending AccessControlPrice and implementing purchase methods.
 */
abstract contract ERC1155AccessControlPurchaseWithERC20 is ERC1155AccessControlPrice, IERC1155PurchaseWithERC20 {
    ...
    // Events are inherited from IERC1155PurchaseWithERC20
    ...
    function purchaseWithERC20(address token, uint256 id, uint256 amount)
        ...
        returns (bool)
    {
        _purchaseWithERC20For(token, _msgSender(), id, amount);
        return true;
    }
    ...
    function _purchaseWithERC20For(address token, address receiver, uint256 id, uint256 amount) internal virtual {
        ...
        if (totalPrice > 0) {
            ...
            if (balance < totalPrice) {
                revert ERC1155PriceInsufficientERC20PaymentTokenBalance(token, totalPrice, balance);
            }

            uint256 allowance = paymentToken.allowance(_msgSender(), address(this));
            if (allowance < totalPrice) {
                revert ERC1155PriceInsufficientERC20PaymentTokenAllowance(token, totalPrice, allowance);
            }

            // Transfer ERC-20 tokens from buyer to treasury
            paymentToken.safeTransferFrom(_msgSender(), _getTreasury(), totalPrice);
        }
