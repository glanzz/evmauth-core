 ...
 */
abstract contract ERC1155Price is ReentrancyGuard, ERC1155, IERC1155Price {
    ...
    // Errors
    error ERC1155PriceInvalidAmount(uint256 amount);
    error ERC1155PriceInvalidReceiver(address receiver);
    error ERC1155PriceInvalidTreasury(address treasury);
    error ERC1155PriceTokenPriceNotSet(uint256 id);
    ...
    function supportsInterface(bytes4 interfaceId) public view virtual override(ERC1155, IERC165) returns (bool) {
        return interfaceId == type(IERC1155Price).interfaceId || super.supportsInterface(interfaceId);
    }
    ...
    function priceOf(uint256 id) external view returns (uint256) {
        if (!_priceConfigs[id].isSet) {
            ...
            revert ERC1155PriceTokenPriceNotSet(id);
        }
    ...
    function _validatePurchase(address receiver, uint256 id, uint256 amount) internal view returns (uint256) {
        if (receiver == address(0)) {
            revert ERC1155PriceInvalidReceiver(receiver);
        }
        if (amount == 0) {
            revert ERC1155PriceInvalidAmount(amount);
        }
        if (!_priceConfigs[id].isSet) {
            revert ERC1155PriceTokenPriceNotSet(id);
        }
    ...
    function _setTokenPrice(uint256 id, uint256 price) internal {
        ...
        emit ERC1155PriceUpdated(_msgSender(), id, price);
    }
    ...
    function _suspendTokenPrice(uint256 id) internal {
        if (_priceConfigs[id].isSet) {
            ...
            emit ERC1155PriceSuspended(_msgSender(), id);
        }
    }
    ...
    function _setTreasury(address payable treasuryAccount) internal virtual {
        if (treasuryAccount == address(0)) {
            revert ERC1155PriceInvalidTreasury(treasuryAccount);
        }
