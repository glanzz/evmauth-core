 ...
 * This contract combines {EVMAuth6909} with the {TokenTTL} mixin, which adds automatic token expiry
 * for any token type that has a time-to-live (TTL) set.
 */
contract EVMAuth6909T is EVMAuth6909, TokenTTL {
    /**
    ...
    function initialize(uint48 initialDelay, address initialDefaultAdmin, string memory uri_)
        ...
        initializer
    {
        __EVMAuth6909T_init(initialDelay, initialDefaultAdmin, uri_);
    }
     ...
     * @param uri_ The URI for the contract; see also: https://eips.ethereum.org/EIPS/eip-6909#content-uri-extension
     */
    function __EVMAuth6909T_init(uint48 initialDelay, address initialDefaultAdmin, string memory uri_)
        public
        onlyInitializing
    {
     ...
     */
    function __EVMAuth6909T_init_unchained() public onlyInitializing {
