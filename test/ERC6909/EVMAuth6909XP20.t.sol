...
/**
 * @dev Mock contract for testing {EVMAuth6909XP20}.
 */
contract MockEVMAuth6909XP20 is EVMAuth6909XP20 {
    function initialize(
        uint48 initialDelay,
...
/**
 * @dev Test contract for {EVMAuth6909XP20}.
 */
contract EVMAuth6909XP20Test is BaseTestWithAccessControlAndERC20s {
    MockEVMAuth6909XP20 internal token;

    // =========== Test Setup ============ //

    function _deployNewImplementation() internal override returns (address) {
        return address(new MockEVMAuth6909XP20());
    }

    function _getContractName() internal pure override returns (string memory) {
        return "EVMAuth6909XP20.t.sol:MockEVMAuth6909XP20";
    }

    function _getInitializerData() internal view override returns (bytes memory) {
        return abi.encodeCall(
            MockEVMAuth6909XP20.initialize, (2 days, owner, "https://example.com/contract-metadata", treasury)
        );
    }

    function _setToken(address proxyAddress) internal override {
        token = MockEVMAuth6909XP20(proxyAddress);
    }
