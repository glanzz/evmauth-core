...
contract ERC6909X_Test is BaseTest {
    ERC6909X internal token;
    ...
    function setUp() public virtual override {
        ...
        ERC6909X implementation = new ERC6909X();

        // Prepare initialization data
        bytes memory initData =
            abi.encodeWithSelector(ERC6909X.initialize.selector, 2 days, owner, "https://token-cdn-domain/{id}.json");

        // Deploy proxy and initialize
        address proxyAddress = deployProxy(address(implementation), initData);
        token = ERC6909X(proxyAddress);
