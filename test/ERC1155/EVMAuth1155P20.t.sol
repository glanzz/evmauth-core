...
/**
 * @dev Mock contract for testing {EVMAuth1155P20}.
 */
contract MockEVMAuth1155P20 is EVMAuth1155P20 {
    function initialize(
        uint48 initialDelay,
...
/**
 * @dev Test contract for {EVMAuth1155P20}.
 */
contract EVMAuth1155P20Test is BaseTestWithAccessControlAndERC20s {
    MockEVMAuth1155P20 internal token;

    // =========== Test Setup ============ //

    function _deployNewImplementation() internal override returns (address) {
        return address(new MockEVMAuth1155P20());
    }

    function _getContractName() internal pure override returns (string memory) {
        return "EVMAuth1155P20.t.sol:MockEVMAuth1155P20";
    }

    function _getInitializerData() internal view override returns (bytes memory) {
        return abi.encodeCall(
            MockEVMAuth1155P20.initialize, (2 days, owner, "https://example.com/api/token/{id}.json", treasury)
        );
    }

    function _setToken(address proxyAddress) internal override {
        token = MockEVMAuth1155P20(proxyAddress);
    }
